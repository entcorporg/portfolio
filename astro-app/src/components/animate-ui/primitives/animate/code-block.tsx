...
function CodeBlock({
  ...
  // Déterminer le thème actuel (utiliser activeTheme du système ou fallback sur le prop theme)
  const currentTheme = (activeTheme as 'light' | 'dark') || theme;

  // Effet séparé pour le highlighting qui se déclenche quand le thème change
  React.useEffect(() => {
    // Attendre que le code soit disponible (soit visibleCode pour animation, soit code complet)
    const codeToHighlight = writing ? visibleCode : code;
    if (!codeToHighlight.length) return;
    ...
    loadHighlightedCode();
  }, [code, visibleCode, lang, themes, writing]);
